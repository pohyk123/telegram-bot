...
def get_responses(updates):
    ...
    for update in updates['result']:
        text = update['message']['text']
        chat = update['message']['chat']['id']
        response = {'response':intents.processIntent(intents.detectIntent(text)),'chat_id':chat}
...
def reply_to_sender(responses_message_only):
    for response in responses_message_only:
        send_message(response['response'],response['chat_id'])
...
def main():
    ...
    while True:
        ...
        if len(updates['result']) > 0:
            ...
            responses_message_only = get_responses_message(updates)
            reply_to_sender(responses_message_only)
